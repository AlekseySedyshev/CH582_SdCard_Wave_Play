...
#define CS_H() R32_PA_OUT |= GPIO_Pin_12
#define CS_L() R32_PA_OUT &=(~GPIO_Pin_12)

#define DO (R32_PB_PIN & GPIO_Pin_15) /* Test MMC DO (high:true, low:false) */

extern volatile BYTE new_sample;
/*--------------------------------------------------------------------------
...
static void xmit_mmc(   // Transmit single  byte
...
static BYTE rcvr_mmc(void)  // Receive single byte from the MMC
{
	BYTE r;

	R8_SPI1_CTRL_MOD &= ~RB_SPI_FIFO_DIR;
	R8_SPI0_BUFFER = 0xFF; // send ff
	while(!(R8_SPI0_INT_FLAG & RB_SPI_FREE));
	R8_SPI1_CTRL_MOD |=RB_SPI_FIFO_DIR;
	//while(R8_SPI0_INT_FLAG & RB_SPI_)
	r= R8_SPI0_BUFFER;
	return r;
}
static void skip_mmc(   //Skip bytes on the MMC
...
static void rcvr_buf(BYTE *buf_ptr, WORD qnt)
{
    uint16_t readlen;

        readlen = qnt;
        R8_SPI1_CTRL_MOD |= RB_SPI_FIFO_DIR; // 设置数据方向为输入
        R16_SPI1_TOTAL_CNT = qnt;            // 设置需要接收的数据长度，FIFO方向为输入长度不为0则会启动传输 */
        R8_SPI1_INT_FLAG = RB_SPI_IF_CNT_END;
        while(readlen)
        {
            if(R8_SPI1_FIFO_COUNT)
            {
                *buf_ptr = R8_SPI1_FIFO;
                buf_ptr++;
                readlen--;
            }
        }
...
static void release_spi(void)   //Deselect the card and release SPI bus
{
	CS_H();
	rcvr_mmc();
}
static BYTE send_cmd(   //Send a command packet to MMC
